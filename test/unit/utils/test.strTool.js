...
describe('可测试的字符串处理模块', function (){
    ...
    it('获取求购信息求购时间文案 一个月内收购', function (done){
        expect(getBuyTime(window.parseInt(new window.Date().getTime() / 1000, 10) + 60 * 60 * 24 * 20)).to.be.equal('长期收购');
        done();
    });

    it('获取求购信息求购时间文案 两个月内收购', function (done){
        expect(getBuyTime(window.parseInt(new window.Date().getTime() / 1000, 10) + 60 * 60 * 24 * 40)).to.be.equal('长期收购');
        done();
    });

    it('获取求购信息求购时间文案 三个月内收购', function (done){
        expect(getBuyTime(window.parseInt(new window.Date().getTime() / 1000, 10) + 60 * 60 * 24 * 70)).to.be.equal('长期收购');
        done();
    });
});
